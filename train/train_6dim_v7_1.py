"""
Phigros v7.1 — 维度贡献修正版
  配置 ×0.55 (2.31→1.27) | 读谱 ×2.00 (0.16→0.32)
  密度 ×1.20 | 位移 ×1.10 | 耐力 ×1.00
"""
import os, sys
...
print('='*70)
print('  Phigros 难度预测系统 v7.1 — 维度贡献修正版')
...
# v6原始co: 密度0.42,0.05,0.05 | 位移0.22,0.10,0.10,0.08 | 配置20个共2.31 | 耐力9个共0.76 | 读谱7个共0.16
# 乘以因子后:
# 密度: 0.504, 0.060, 0.060 = 0.624
# 位移: 0.242, 0.110, 0.110, 0.088 = 0.550
# 配置: 所有×0.55, 原2.31→1.271
# 耐力: 不变 0.76
# 读谱: 所有×2.00, 原0.16→0.32
FLAT_FEATURES = [
    # === 密度 (×1.20) ===
    ...
    # === 配置 (×0.55) ===
    ('stair_density', 1.0, 0.099),           # 0.18→
    ('stair_speed_avg', 8.0, 0.0825),        # 0.15→
    ('stair_complexity', 0.2, 0.055),        # 0.10→
    ('stair_chord_ratio', 0.3, 0.044),       # 0.08→
    ('trill_density', 2.0, 0.055),           # 0.10→
    ('jack_density', 2.0, 0.066),            # 0.12→
    ('chord_size_entropy', 0.5, 0.1375),     # 0.25→
    ('sim_pos_spread_mean', 1.0, 0.055),     # 0.10→
    ('multi_finger_3plus_events', 10, 0.0275), # 0.05→
    ('chord_alternation_rate', 0.5, 0.0935), # 0.17→
    ('weighted_mf_score_per_sec', 10, 0.0935),# 0.17→
    ('discrete_mf_ratio', 0.3, 0.066),       # 0.12→
    ('position_cluster_count', 4, 0.066),    # 0.12→
    ('track_deviation_score', 0.3, 0.044),   # 0.08→
    ('position_entropy', 2.0, 0.055),        # 0.10→
    ('position_range_used', 0.5, 0.033),     # 0.06→
    ('pattern_switch_rate', 1.0, 0.055),     # 0.10→
    ('direction_irregularity', 0.5, 0.044),  # 0.08→
    ('hold_interference_index', 0.3, 0.055), # 0.10→
    ('drag_flick_ratio', 0.3, 0.044),        # 0.08→
    # === 耐力 (×1.00) ===
    ...
    # === 读谱 (×2.00) ===
    ('density_transition_mean', 0.15, 0.16),  # 0.08→
    ('density_transition_std', 0.2, 0.16),    # 0.08→
    ('tempo_change_count', 50, 0.16),         # 0.08→
    ('offbeat_ratio', 0.04, 0.16),            # 0.08→
    ('rhythm_entropy', 2.5, 0.12),            # 0.06→
    ('type_switch_per_sec', 0.4, 0.12),       # 0.06→
    ('note_clutter_ratio', 0.05, 0.12),       # 0.06→
]
...
out_path = os.path.join(os.path.dirname(__file__), 'models', '6dim_model_v7_1.pkl')
