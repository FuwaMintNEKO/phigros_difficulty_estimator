"""
v8.4: 耐力维度高潮段占比替换 stamina_ratio
  - 移除 stamina_ratio（co=0.0001 死特征）
  - 新增 above_avg_density_ratio（跟密度维度同一窗口，rcnps阈值）
  - 耐力维度核心 = 高潮段占比（1秒窗口≥rcnps的窗口比例）
"""
import os, sys
...
print('='*70); print('  v8.4 — 耐力=高潮段占比(above_avg_density_ratio)'); print('='*70)

with open('models/6dim_model_v8_3.pkl', 'rb') as f:
    v83 = pickle.load(f)
P95o = v83['p95_vals']; P99o = v83['p99_vals']
DC = v83['dynamic_cap']; FLATo = v83['FLAT_FEATURES']
FNo = v83['feature_names']
...
for feat in ['above_avg_density_ratio', 'above_avg_density_mean', 'density_dimension']:
    vals = [f.get(feat, 0) for f in feats_list]
    P95[feat] = float(np.percentile(vals, 95))
    P99[feat] = float(np.percentile(vals, 99))
    print(f'{feat}: P95={P95[feat]:.4f} P99={P99[feat]:.4f}')

# FLAT_FEATURES: 替换 stamina_ratio → above_avg_density_ratio
FLAT = []
for fname, bl, co in FLATo:
    if fname == 'stamina_ratio':
        # 替换为 above_avg_density_ratio，初始 co 让 Ridge 学习
        FLAT.append(('above_avg_density_ratio', 0.30, 0.01))
        print('替换: stamina_ratio → above_avg_density_ratio')
    else:
        FLAT.append((fname, bl, co))
...
for it in range(3):
    ...
    for k in ['above_avg_density_ratio','tap_per_second','total_notes','duration_sec','rest_ratio']:
...
if best:
    ...
    with open('models/6dim_model_v8_3.pkl', 'rb') as f:
        v83m = pickle.load(f)
    gb_o = v83m['gb']; sc_o = v83m['scaler']; fn_o = v83m['feature_names']
    flat_o = v83m['FLAT_FEATURES']; dc_o = v83m['dynamic_cap']
    p95_o = v83m['p95_vals']; p99_o = v83m['p99_vals']
    
    key_tags = ['Final EndGame','朧月','Apollo','恋ひ恋ふ縁','怪文書','Waking','ex7','ex8','Regrets','silly','Submerged','Cheerio']
    
    print(f'\n{"谱面":<14} {"定数":>5} {"v8.3":>7} {"v8.4":>7} {"Δ":>7} {"耐力":>6} {"高潮%":>6}')
    print('-' * 68)
    ...
    for fn, feats, rating in chart_data:
        ...
        for fname, bl, co in flat_o:
            ...
            if v <= th: continue
            e = (v/th - 1.0) ** 0.70
            if v > max(p99_o.get(fname,0), bl * 0.5):
                pe = (v / max(p99_o.get(fname,0), bl * 0.5) - 1.0)
                e += 0.5 * max(0, pe) ** 0.70
            po_raw += co * e
        ...
        short = fn[:14]
        aa = feats.get('above_avg_density_ratio', 0)
        print(f'{short:<14} {rating:>5.1f} {po_pred:>7.2f} {pn_pred:>7.2f} {pn_pred-po_pred:>+7.2f} {po_pred-po_gb:>6.1f} {aa:>6.0%}')
...
with open('models/6dim_model_v8_4.pkl', 'wb') as f: pickle.dump(out, f)
print(f'\n已保存: models/6dim_model_v8_4.pkl')
