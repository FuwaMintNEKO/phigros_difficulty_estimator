...
print('='*70)
print('  Phigros 难度预测系统 v5.5（真实密度+休息段）')
...
# ====== 平铺特征定义（模块级别，供保存和复用） ======
FLAT_FEATURES = [
    # === 密度 (Density — 真实密度为主，排除>1s间隙) ===
    ('real_core_notes_per_second', 3.0, 0.18),  # 真实核心TPS（排除>1s间隙）
    ('core_notes_per_second', 3.0, 0.04),  # (旧)全时段核心TPS，辅助参考
    ('real_notes_per_second', 3.0, 0.04),  # 真实NPS
    ('notes_per_second', 3.0, 0.02),  # (旧)全时段NPS
    ('core_peak_density_top5avg_1beat', 0.5, 0.20),  # core版
    ('core_density_above_zero_ratio', 0.6, 0.08),  # core版
    ('core_std_density_1beat', 0.3, 0.08),  # core版
    
    # === 1smax密度 (1s Max Density — 核心: tap+hold) ===
    ('core_peak_density_1sec_top5avg', 8, 0.22),
    ('peak_density_1sec_top5avg', 8, 0.06),  # 全音符峰值退化(NPS参考)
    ('peak_tps_1sec_top5avg', 8, 0.18),
    ('core_micro_top5_0.0625beat', 0.5, 0.08),  # core版
    
    # === 平均位移 (Movement) ===
    ('movement_per_second', 3.0, 0.18),
    ...
    # === 读谱 (Reading) ===
    ('density_transition_mean', 0.15, 0.08),
    ('density_transition_std', 0.2, 0.08),
    ('tempo_change_count', 50, 0.08),
...
out_path = os.path.join(os.path.dirname(__file__), 'models', '5dim_model_v5_5.pkl')
...
# 保存CSV
import csv
csv_path = out_path.replace('.pkl', '_predictions_v5_4.csv')
