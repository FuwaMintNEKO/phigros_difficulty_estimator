"""
v7.6: 删total_notes(GB) + 加real_active_sec到耐力boost

改动:
  1. GB特征列表移除 total_notes (之前占24%重要性)
     → GB被迫用密度/配置/位移来估计难度
     → 键盘谱(高配置中密度)vs长谱(高密度低配置)能更好区分
  2. Boost耐力维度加 real_active_sec (有效时长)
     → 补充被total_notes移除后丢失的时长信息
  3. max_depth=5不变, 其它架构同v7.3
"""
import os, sys
...
print('='*70); print('  v7.6 — 删total_notes(GB) + 加real_active_sec(boost)'); print('='*70)
...
# ── 1. GB特征: 删total_notes ──
GB_REMOVE = {'total_notes', 'tap_count'}  # tap_count和total_notes高度相关
FN_NEW = [f for f in FN_OLD if f not in GB_REMOVE]
print(f'GB特征: {len(FN_OLD)} → {len(FN_NEW)} (删除 {GB_REMOVE})')

# ── 2. Boost: 加real_active_sec到耐力 ──
old_bl = {f[0]: f[1] for f in FLAT_OLD}
old_co = {f[0]: f[2] for f in FLAT_OLD}
EXTRA_BOOST = [('real_active_sec', 60.0, 0.03)]  # 基线60秒有效时长, 初始co很小
FLAT_NEW = list(FLAT_OLD) + EXTRA_BOOST
for fn, bl, co in EXTRA_BOOST:
    old_bl[fn] = bl; old_co[fn] = co
feat_names_boost = [f[0] for f in FLAT_NEW]
print(f'Boost特征: {len(FLAT_OLD)} → {len(FLAT_NEW)} (+real_active_sec)')
...
for it in range(2):
    ...
    # 看real_active_sec的co
    ra_idx = feat_names_boost.index('real_active_sec')
    print(f'  real_active_sec co={co_new[ra_idx]:.4f} (after Ridge)')
    print(f'  density_dim co={co_new[feat_names_boost.index("density_dimension")]:.4f}')
...
top5 = sorted(imp_f.items(), key=lambda x: -x[1])[:5]
print(f'  Top5 GB: {" ".join(f"{k}={v:.3f}" for k,v in top5)}')
...
for target in [0.18, 0.20, 0.22, 0.24, 0.26]:
    for power in [0.60, 0.65, 0.70, 0.75, 0.80, 0.85]:
        for thresh in [0.18, 0.20, 0.22, 0.24, 0.26, 0.28]:
            ...
            if b <= 6 and m < 0.55:
                mark = ' ***' if b <= 2 else ' *'
                print(f'  t={target:.2f} p={power:.2f} th={thresh:.2f} MAE={m:.3f} +{p}/-{n} bal={b}{mark}')
...
if best_sig:
    ...
    key_tags = ['Final EndGame', '朧月', 'Apollo', '恋ひ恋ふ縁', '怪文書', 'Waking', 'Submerged City', 'The Final']
    key_short = ['FinalEnd', '胧月', 'Apollo', '恋', '怪文書', 'Waking', 'Submerged', 'FinalEnd2']
    
    print(f'\n{"谱面":<15} {"定数":>5} {"v7.3":>7} {"v7.6":>7} {"Δpred":>7} {"ΔGB":>7}')
    print('-' * 56)
    
    for fn, feats, rating in chart_data:
        matched_tag = None
        for pat, tag in zip(key_tags, key_short):
            if pat in fn and matched_tag is None:
                matched_tag = tag; break
        if matched_tag is None: continue
        ...
        print(f'{matched_tag:<15} {rating:>5.1f} {po_pred:>7.2f} {pn_pred:>7.2f} {pn_pred-po_pred:>+7.2f} {pn_gb-po_gb:>+7.2f}')
...
with open('models/6dim_model_v7_6.pkl', 'wb') as f: pickle.dump(out, f)
print(f'\n已保存: models/6dim_model_v7_6.pkl\n{"="*70}')
