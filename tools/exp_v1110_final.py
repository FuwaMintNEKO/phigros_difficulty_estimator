# -*- coding: utf-8 -*-
"""段降权(14-16.5) x 校准表(0.51/0.36/0.16 趋势) 全段验证"""
...
def predict_cfg(feats_raw, level, calib, win_hi=16.5, stack_on=True):
    ...
    for fname, bl, co in FLAT:
        ...
        total += x_
    pred = p_gb + total
    if stack_on and is_stack and 14 < pred <= win_hi:
        pred -= total * 0.08
...
for tag, cal, win, st in [
    ('段降权14-16.5 + 0.51/0.36/0.16 (v11.9趋势)', CAL9, 16.5, True),
    ('段降权14-16.5 + 0.41/0.26/0.06 (现app)', CAL10, 16.5, True),
    ('无降权 + 0.51/0.36/0.16', CAL9, 16.5, False),
]:
    ps = np.array([predict_cfg(r['feats'], r['level'], cal, win, st) for r in ranked])
    errs = ps - ds
    segs = []
    ...
    print(f'{tag:<42} MAE={np.abs(errs).mean():.3f} bias={errs.mean():+.3f}')
    print(f'    {" ".join(segs)}')
print('DONE')