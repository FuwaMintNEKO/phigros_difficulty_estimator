# -*- coding: utf-8 -*-
"""高段过降修复: 校准表加17+段/调16-17, 保持MAE改善同时修复17+低估
"""
...
CONFIGS = [
    ('v11.8b当前', CUR),
    ('+17段-0.25(加回)', [(14,15,0.51),(15,16,0.36),(16,17,0.16),(17,99,-0.25)]),
    ('+17段-0.4', [(14,15,0.51),(15,16,0.36),(16,17,0.16),(17,99,-0.40)]),
    ('16-17改0.10', [(14,15,0.51),(15,16,0.36),(16,17,0.10)]),
    ('16-17改0.10+17段-0.3', [(14,15,0.51),(15,16,0.36),(16,17,0.10),(17,99,-0.30)]),
    ('15-16改0.32+16-17改0.12', [(14,15,0.51),(15,16,0.32),(16,17,0.12)]),
]
for tag, cal in CONFIGS:
    ps = np.array([predict(r['feats'], r['level'], cal) for r in ranked])
    errs = ps - ds
    rho, _ = spearmanr(ds, ps)
    seg = []
    for lo, hi, t2 in [(14,15,'14-15'),(15,16,'15-16'),(16,17,'16-17'),(17,99,'>=17')]:
        mk = np.where((ds >= lo) & (ds < hi))[0]
        if len(mk): seg.append(f'{t2}:{errs[mk].mean():+.2f}/{np.abs(errs[mk]).mean():.2f}')
    print(f'{tag:<24} MAE={np.abs(errs).mean():.3f} bias={errs.mean():+.3f} rho={rho:.3f} | {" ".join(seg)}')
print('DONE')
