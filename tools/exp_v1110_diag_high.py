...
def predict_with(feats_raw, level, use_stack, calib):
    ...
    high = {'叠键', '多押', '变速', '位移'}
    stack = 0.92 if (use_stack and sum(1 for t in ts if t in high) >= 2) else 1.0
...
CONFIGS = [
    ('生产(降权+0.41/0.26/0.06)', True, [(14,15,0.41),(15,16,0.26),(16,17,0.06)]),
    ('无降权+0.41/0.26/0.06', False, [(14,15,0.41),(15,16,0.26),(16,17,0.06)]),
    ('无降权+0.51/0.36/0.16', False, [(14,15,0.51),(15,16,0.36),(16,17,0.16)]),
    ('降权+0.51/0.36/0.16', True, [(14,15,0.51),(15,16,0.36),(16,17,0.16)]),
]
for tag, use_s, cal in CONFIGS:
    ps = np.array([predict_with(r['feats'], r['level'], use_s, cal) for r in ranked])
    errs = ps - ds
    seg = []
    ...
    print(f'{tag:<24} MAE={np.abs(errs).mean():.3f} bias={errs.mean():+.3f} | {" ".join(seg)}')
# Bathin 各配置
print('\nBathin:')
for tag, use_s, cal in CONFIGS:
    for r in ranked:
        if 'Bathin' in r['name']:
            pr = predict_with(r['feats'], r['level'], use_s, cal)
            print(f'  {tag}: {pr:.2f} err={pr-round(r["diff"],1):+.2f}')
            break
print('DONE')
