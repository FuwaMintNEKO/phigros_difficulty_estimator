# -*- coding: utf-8 -*-
"""相同校准对比: v11.9 vs v11.10 (都用 0.51/0.36/0.16) + 16-16.5偏高谱
"""
...
# 复用 exp_v1110_compare_high.py 的 make_predictor — 直接重新实现(简化)
...
CAL = [(14,15,0.51),(15,16,0.36),(16,17,0.16)]  # 相同校准
...
p9 = make_predictor(m9, CAL); p10 = make_predictor(m10, CAL)
with open(os.path.join(_ROOT, 'data', 'phira', 'feats_cache_v11.pkl'), 'rb') as f:
    cache = pickle.load(f)
ranked = [r for r in cache['ranked'] if r['diff'] and r['diff'] > 10]
ds = np.array([round(r['diff'],1) for r in ranked])
ps9 = np.array([p9(r['feats'], r['level']) for r in ranked])
ps10 = np.array([p10(r['feats'], r['level']) for r in ranked])
e9 = ps9 - ds; e10 = ps10 - ds
print('===== 相同校准(0.51/0.36/0.16) 分段对比 =====')
for lo, hi, tag in [(10,14,'<14'),(14,15,'14-15'),(15,16,'15-16'),(16,16.5,'16-16.5'),(16.5,17,'16.5-17'),(17,99,'>=17')]:
    mk = np.where((ds >= lo) & (ds < hi))[0]
    if len(mk):
        print(f'  [{tag}]: n={len(mk):3d} | v11.9: {e9[mk].mean():+.3f} | v11.10: {e10[mk].mean():+.3f} | Δ={e10[mk].mean()-e9[mk].mean():+.3f}')
print(f'整体: v11.9 MAE={np.abs(e9).mean():.3f}/bias={e9.mean():+.3f}/rho={spearmanr(ds,ps9)[0]:.3f} | v11.10 MAE={np.abs(e10).mean():.3f}/bias={e10.mean():+.3f}/rho={spearmanr(ds,ps10)[0]:.3f}')
# 16-16.5 段 err>0.5 的谱 (v11.10)
print('\n16-16.5 段 |err|>0.5 (v11.10):')
mk = np.where((ds >= 16) & (ds < 16.5))[0]
for i in mk:
    if abs(e10[i]) > 0.5:
        print(f'  {ranked[i]["name"][:24]:<26} 定数={ds[i]:.1f} v11.9={ps9[i]:.2f}({e9[i]:+.2f}) v11.10={ps10[i]:.2f}({e10[i]:+.2f})')
print('DONE')
