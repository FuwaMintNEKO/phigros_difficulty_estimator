...
# 预计算锚点/全量的gb与相关量? 直接循环
results = []
grid = {
  'drag_per_sec': [0.3, 0.5, 0.7, 1.0],
  'density_transition_std': [0.3, 0.5, 0.7, 1.0],
  'jack_max_run': [0.3, 0.5, 0.7, 1.0],
  'eff_peak_tps_1s': [1.0, 1.2, 1.4],
...
for trial in range(400):
    ...
    # 分数: 锚点MAE为主, 全量MAE为次 (全量不能超0.545)
    if errs <= 0.545:
        score = errs_a + (errs - 0.532) * 2.0
        best.append((score, errs_a, errs, ov))
best.sort(key=lambda x: x[0])
print('top15 (score | 锚MAE 全MAE | 权重):')
for sc, ea, em, ov in best[:15]:
    print(f'score={sc:.3f} 锚MAE={ea:.3f} 全MAE={em:.3f} | ' + ' '.join(f'{k}={v}' for k, v in ov.items()))
print('DONE')