# -*- coding: utf-8 -*-
"""应用jline P95修正后的完整验证 (锚点+全量+Feeling Blue)"""
...
# 应用 P95 修正
_P95_FIX = {'jline_movement_density': 107.1, 'jline_rotate_density': 18.6, 'jline_disappear_density': 15.1}
for _k, _v in _P95_FIX.items():
    if _k in app_mod.P95:
        app_mod.P95[_k] = _v
...
for r in ranked:
    if r['id'] in (47264, 21041, 580, 8706):
        print(f"  {r['name'][:20]:<22} id={r['id']} diff={round(r['diff'],1)} 预测={predict_full(r['feats'], r['level']):.2f}")
print('\n=== 全量 ===')
