...
def predict(feats_raw, level='IN', stack_adj=0.0, burst_adj=0.0, burst_lo=16.0):
    ...
    if stack_adj and len(ts) >= 4 and 14 < pred <= 16:
        pred -= stack_adj
    if burst_adj and ('32分' in ts or '爆发' in ts) and pred >= burst_lo:
        pred += burst_adj
    return pred
...
CONFIGS = [
    ('基线', 0, 0, 16),
    ('堆-0.3/爆+0.3@17', 0.3, 0.3, 17),
    ('堆-0.3/爆+0.2@16.5', 0.3, 0.2, 16.5),
    ('堆-0.25/爆+0.25@17', 0.25, 0.25, 17),
    ('堆-0.2/爆+0.2@16.5', 0.2, 0.2, 16.5),
    ('堆-0.3/爆+0.4@17.2', 0.3, 0.4, 17.2),
]
for tag, sa, ba, bl_ in CONFIGS:
    ps = np.array([predict(r['feats'], r['level'], sa, ba, bl_) for r in ranked])
    errs = ps - ds
