# -*- coding: utf-8 -*-
"""网格搜索: 降权窗口上限 x 16-17校准 x 16.5+补偿, 目标三段bias均衡"""
...
def predict_cfg(feats_raw, level, win_hi, calib, comp=0.0, comp_cond=None):
    ...
    pred = p_gb + total
    # 段降权: 14 < pred <= win_hi
    if is_stack and 14 < pred <= win_hi:
        pred -= total * 0.08
    ...
    # 16.5+ 补偿: 预测>16.5 且条件满足
    if comp > 0 and pred > 16.5:
        ok = True
        if comp_cond == 'mf3': ok = mf3 >= 25
        elif comp_cond == 'any': ok = True
        if ok: pred += comp
...
import itertools
results = []
for win_hi, adj17, comp, cond in itertools.product(
        [16.0, 16.5, 17.0], [0.06, 0.16, 0.26], [0.0, 0.10], ['none', 'mf3']):
    calib = [(14,15,0.41),(15,16,0.26),(16,17,adj17)]
    ps = np.array([predict_cfg(r['feats'], r['level'], win_hi, calib, comp, cond if comp>0 else None) for r in ranked])
    errs = ps - ds
    segs = []
    ...
    results.append((mae, win_hi, adj17, comp, cond, segs))
results.sort()
print('top12 (MAE | win adj comp cond | segs):')
for mae, w, a, c, cd, segs in results[:12]:
    print(f'MAE={mae:.3f} win={w} adj17={a} comp={c}({cd}) | {" ".join(segs)}')
print('\n全量(按win/adj17分组, comp=0):')
for w in [16.0, 16.5, 17.0]:
    for a in [0.06, 0.16, 0.26]:
        for mae, w2, a2, c2, cd2, segs in results:
            if w2 == w and a2 == a and c2 == 0.0:
                print(f'win={w} adj17={a}: MAE={mae:.3f} | {" ".join(segs)}')
                break
print('DONE')