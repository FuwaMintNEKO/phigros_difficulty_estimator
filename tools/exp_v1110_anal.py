# -*- coding: utf-8 -*-
"""16-16.5 vs 16.5+ 预测结构分析: 预测值/GB/boost/校准段分布"""
...
def predict_raw(feats_raw, level, use_stack=True, calib=[(14,15,0.41),(15,16,0.26),(16,17,0.06)]):
    """返回 (pred无校准, gb, total_boost, is_stack)"""
...
for lo, hi, tag in [(16,16.5,'16-16.5'),(16.5,17,'16.5-17'),(17,99,'>=17')]:
    mk = [(i,r) for i,r in enumerate(ranked) if lo <= ds[i] < hi]
    print(f'=== {tag} n={len(mk)} ===')
    rows = []
    for i, r in mk:
        pred0, pgb, tot, st = predict_raw(r['feats'], r['level'])
        rows.append((r['name'], ds[i], pred0, pgb, tot, st))
    # 预测值统计
    preds = np.array([x[2] for x in rows])
    print(f'  预测(无校准)分布: min={preds.min():.2f} p25={np.percentile(preds,25):.2f} med={np.median(preds):.2f} p75={np.percentile(preds,75):.2f} max={preds.max():.2f}')
    # 哪些预测值落在16-17校准段内
    in1617 = np.sum((preds > 16) & (preds <= 17))
    print(f'  落在16-17校准段内: {in1617}/{len(rows)}')
    st_n = sum(1 for x in rows if x[5])
    print(f'  堆料标签>=2: {st_n}/{len(rows)}')
    for r in sorted(rows, key=lambda x: x[1] - x[2])[:8]:
        print(f'    {r[0][:20]:<20} 真实={r[1]:.2f} 预测={r[2]:.2f} gb={r[3]:.2f} boost={r[4]:.2f} stack={r[5]} err={r[1]-r[2]:+.2f}')
print('DONE')