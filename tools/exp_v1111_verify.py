# -*- coding: utf-8 -*-
"""验证: app.predict_one_chart(完整管线) 与实验脚本一致性 + 段统计"""
...
# 用 app_mod 的完整预测路径: 需要构造 chart_data? predict_one_chart 需要 chart_data。
# 直接用 app_mod 内部的各环节组合: gb + compute_boost + 定轨 + hold + 段降权 + 校准
...
print(f'v11.11生产管线 MAE={np.abs(errs).mean():.3f} bias={errs.mean():+.3f}')
print('  ' + ' '.join(segs))
print('app.VERSION =', app_mod.VERSION)
print('app._CALIB_TABLE =', app_mod._CALIB_TABLE)
print('DONE')