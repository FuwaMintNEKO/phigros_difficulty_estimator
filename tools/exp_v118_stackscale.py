...
def predict(feats_raw, level='IN', stack_scale=1.0, calib=None):
    ...
    for fname, bl, co in FLAT:
        ...
        if is_stack: co2 = co2 * stack_scale
        x_ = co2 * (e**0.70)
...
CUR = [(14,15,0.55),(15,16,0.40),(16,17,0.20)]
CONFIGS = [
    ('现状', 1.0, CUR),
    ('×0.95', 0.95, CUR),
    ('×0.95 + 校准-0.04', 0.95, [(14,15,0.51),(15,16,0.36),(16,17,0.16)]),
    ('×0.95 + 校准-0.02', 0.95, [(14,15,0.53),(15,16,0.38),(16,17,0.18)]),
    ('×0.92', 0.92, CUR),
]
for tag, sc, cal in CONFIGS:
    ps = np.array([predict(r['feats'], r['level'], sc, cal) for r in ranked])
    errs = ps - ds
