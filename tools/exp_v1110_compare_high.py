# -*- coding: utf-8 -*-
"""v11.9 vs v11.10 高难段对比 (ranked 16.5+)
"""
...
print(f'\n整体: v11.9 MAE={np.abs(e9).mean():.3f} bias={e9.mean():+.3f} rho={spearmanr(ds,ps9)[0]:.3f} | v11.10 MAE={np.abs(e10).mean():.3f} bias={e10.mean():+.3f} rho={spearmanr(ds,ps10)[0]:.3f}')

print('\n===== 16.5+ 逐谱对比 (高难段) =====')
mk = np.where(ds >= 16.5)[0]
rows = []
for i in mk:
    rows.append((ranked[i]['name'], ds[i], ps9[i], ps10[i], e9[i], e10[i]))
rows.sort(key=lambda x: -x[1])
print(f'{"谱":<26}{"定数":>6}{"v11.9":>7}{"v11.10":>7}{"err9":>7}{"err10":>7}')
for name, d, p9v, p10v, e9v, e10v in rows:
    print(f'{name[:24]:<26}{d:>6.1f}{p9v:>7.2f}{p10v:>7.2f}{e9v:>+7.2f}{e10v:>+7.2f}')
print('DONE')
