# -*- coding: utf-8 -*-
"""定向降权: drag_per_sec/std/jack_run (Chart_SP虚高特征, 用户实测drag零操作)"""
...
combos = [
    ('baseline', {}),
    ('drag0.3', {'drag_per_sec':0.3}),
    ('drag0.5+std0.5', {'drag_per_sec':0.5, 'density_transition_std':0.5}),
    ('drag0.3+std0.3+jack0.3', {'drag_per_sec':0.3, 'density_transition_std':0.3, 'jack_max_run':0.3}),
    ('drag0.5+std0.5+jack0.5+longjack0.5', {'drag_per_sec':0.5, 'density_transition_std':0.5, 'jack_max_run':0.5, 'long_jack_count':0.5}),
]
for tag, ov in combos:
    ...
    print(f'{tag:<30} 锚MAE={np.abs(errs_a).mean():.3f} 全MAE={np.abs(errs).mean():.3f}')
    print(f'   锚点: ' + ' '.join(f'{n[:7]}={x:+.2f}' for n, x in zip(names, errs_a)))
    print(f'   全量: {" ".join(segs)}')
