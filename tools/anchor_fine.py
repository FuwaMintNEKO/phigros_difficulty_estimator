...
grid = {
  'drag_per_sec': [0.2, 0.25, 0.3, 0.4, 0.6],
  'density_transition_std': [0.5, 0.7, 1.0],
  'jack_max_run': [0.4, 0.5, 0.7, 1.0],
  'eff_peak_tps_1s': [1.1, 1.2, 1.3, 1.4],
  'above_avg_duration_sec': [1.15, 1.25, 1.3, 1.4],
  'jline_movement_density': [0.4, 0.5, 0.7, 1.0],
  'above_avg_density_mean': [0.8, 0.85, 0.9, 1.0],
  'weighted_mf_score_per_sec': [0.85, 0.9, 1.0],
  'movement_per_second': [1.1, 1.2, 1.3],
...
for trial in range(800):
    ...
    ea = np.abs(ps_a - TGT).mean()
    if ea <= 0.13:
        ps = np.array([predict_full(r['feats'], r['level']) for r in ranked])
        errs = ps - ds
        mae = np.abs(errs).mean()
        m1 = errs[(ds>=14)&(ds<15)].mean()
        m2 = errs[(ds>=15)&(ds<16)].mean()
        m3 = errs[(ds>=16)&(ds<16.5)].mean()
        m4 = errs[(ds>=16.5)&(ds<17)].mean()
        m5 = errs[ds>=17].mean()
        # 目标: 中段(14-16)不更负 + 16.5+不更差
        score = mae + 0.5*abs(m1) + 0.5*abs(m2) + 0.3*(abs(m4)+abs(m5))
        best.append((score, ea, mae, m1, m2, m3, m4, m5, ov, ps_a-TGT))
    app_mod.MANUAL_FLAT = saved
best.sort(key=lambda x: x[0])
print('top10 (score | 锚MAE 全MAE | 段偏差 14-15/15-16/16-16.5/16.5-17/>=17):')
for b in best[:10]:
    print(f'score={b[0]:.3f} 锚MAE={b[1]:.3f} 全MAE={b[2]:.3f} | {b[3]:+.2f}/{b[4]:+.2f}/{b[5]:+.2f}/{b[6]:+.2f}/{b[7]:+.2f}')
    print(f'   锚差: ' + ' '.join(f'{x:+.2f}' for x in b[9]))
    print(f'   权重: ' + ' '.join(f'{k}={v}' for k, v in b[8].items()))
print('DONE')