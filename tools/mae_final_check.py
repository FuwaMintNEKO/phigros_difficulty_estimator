...
# 应用最优权重 (top1: drag0.3 std0.5 jack0.7 eff1.0 dur1.15 jline0.4 dens1.0 wmf0.85 mv1.0 tsw1.0 chordalt0.7 jrot0.8)
OV = {'drag_per_sec':0.3, 'density_transition_std':0.5, 'jack_max_run':0.7, 'eff_peak_tps_1s':1.0,
      'above_avg_duration_sec':1.15, 'jline_movement_density':0.4, 'above_avg_density_mean':1.0,
      'weighted_mf_score_per_sec':0.85, 'movement_per_second':1.0, 'type_switch_per_sec':1.0,
      'chord_alternation_rate':0.7, 'jline_rotate_density':0.8}
...
CAL = [(12,13,-0.2),(13,14,-0.15),(14,15,0.1),(15,16,0.2),(16,16.5,0.2),(16.5,17,0.15),(17,99,0.05)]
...
for r in ranked:
    ...
    elif r['id'] == 15875: print(f"  FREEDOM DiVE: 锚=16.15 预测={predict_full(r['feats'], r['level'], CAL):.2f}")
...
print(f"  Chart_SP#1347: 锚=17.65 预测={predict_full(cs, 'IN', CAL):.2f}")
# 全量
...
print(f'\n过滤集: MAE={np.abs(errs).mean():.3f} bias={errs.mean():+.3f}')
print('  ' + ' '.join(segs))
