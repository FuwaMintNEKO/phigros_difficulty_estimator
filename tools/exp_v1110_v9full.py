# -*- coding: utf-8 -*-
"""v11.9完整管线 vs v11.10完整管线 16.5+段对比 (含堆料降权与各自校准)"""
import os, sys, numpy as np, io, pickle
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
_ROOT = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))
sys.path.insert(0, _ROOT)
import app as app_mod
gb10, scl10, FN10, LV10 = app_mod.gb, app_mod.scaler, app_mod.FN, app_mod.LV_ORDER
...
with open(os.path.join(_ROOT, 'models', '6dim_model_v11_9.pkl'), 'rb') as f:
    m9 = pickle.load(f)
gb9, scl9, fn9, lv9 = m9['gb'], m9['scaler'], m9['feature_names'], m9['lv_order']
FLAT9 = m9['MANUAL_FLAT']; P95_9 = m9['p95_vals']; P99_9 = m9['p99_vals']; CAPS9 = m9['caps']

def predict_full(feats_raw, level, gb, scl, fn, lv_order, FLAT, P95, P99, CAPS, calib, stack_scale=0.92):
    ...
    p_gb = float(gb.predict(scl.transform(x))[0])
    ...
    for fname, bl, co in FLAT:
        ...
        if is_stack and stack_scale < 1.0: co2 = co2 * stack_scale
        x_ = co2 * (e**0.70)
...
CAL9 = [(14,15,0.51),(15,16,0.36),(16,17,0.16)]
CAL10 = [(14,15,0.41),(15,16,0.26),(16,17,0.06)]
CONFIGS = [
    ('v11.9全管线(降权+0.51/0.36/0.16)', gb9, scl9, fn9, lv9, FLAT9, P95_9, P99_9, CAPS9, CAL9, 0.92),
    ('v11.10生产(降权+0.41/0.26/0.06)', gb10, scl10, FN10, LV10, app_mod.MANUAL_FLAT, app_mod.P95, app_mod.P99, app_mod.CAPS, CAL10, 0.92),
    ('v11.10无降权(0.41/0.26/0.06)', gb10, scl10, FN10, LV10, app_mod.MANUAL_FLAT, app_mod.P95, app_mod.P99, app_mod.CAPS, CAL10, 1.0),
]
for tag, gb, scl, fn, lvo, FL, p95, p99, caps, cal, ss in CONFIGS:
    ps = np.array([predict_full(r['feats'], r['level'], gb, scl, fn, lvo, FL, p95, p99, caps, cal, ss) for r in ranked])
    errs = ps - ds
    segs = []
    ...
    print(f'{tag:<36} MAE={np.abs(errs).mean():.3f} bias={errs.mean():+.3f}')
    print(f'    {" ".join(segs)}')
print('DONE')