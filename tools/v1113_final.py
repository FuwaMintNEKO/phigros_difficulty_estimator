...
errs = ps - ds_f
print(f'v11.13最终 过滤集 n={len(ps)} MAE={np.abs(errs).mean():.3f} RMSE={np.sqrt((errs**2).mean()):.3f} bias={errs.mean():+.3f} rho={spearmanr(ps, ds_f).statistic:.3f}')
segs = []
...
for r in ranked:
    ...
    elif r['id'] == 15875: print(f"  FREEDOM: 锚=16.15 预测={predict_full(r['feats'], r['level']):.2f}")
...
print(f"  Chart_SP: 锚=17.65 预测={predict_full(cs, 'IN'):.2f}")
print('DONE')